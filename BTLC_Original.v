...
(** ** Lambda-calculus reduction relation *)
...
Proof.
  ...
  Case "tcase".
       inversion Ht1; inversion Ht2; subst; eauto.
       destruct (IHt1 _ _ _ _ _ H3 H12) as [HL1 HR1]; subst.
       destruct (IHt2 _ _ _ _ _ H6 H15) as [HL2 HR2]; subst.
       destruct (IHt3 _ _ _ _ _ H7 H16) as [HL3 HR3]; subst.
       inversion HL2; subst.
       inversion HL3; subst.
       auto.
Qed.
...
Proof with eauto.
  ...
  Case "T_Case".
    ...
    SCase "ts is a value".
      ...
      destruct H0 as [HL | HR]; [ destruct HL as [t1 Heq]
                                | destruct HR as [t2 Heq]]; subst.
      SSCase "ts = tinl t1 Tr".
        right. eexists; eapply ST_CaseL. inversion H; auto.
      SSCase "ts = tinr t2 Tl".
        right. eexists; eapply ST_CaseR. inversion H; auto.
    SCase "ts steps".
    right. destruct H as [ts' Hstep].
...
Proof with eauto.
  ...
  inversion HSt';subst.
  SCase "ts".
        destruct (IHHTt1 eq_refl _ H3) as [n0 [? ?]].  
        exists (n0 + max (nl + m) (nr + p) + 2).
  ...
  split; [ | eapply T_App;eauto]. 
        ...
        simpl;
        assert (HEQ1: (nl + m + 0 + 1) = S (nl + m))
          by omega; rewrite HEQ1; clear HEQ1;
        assert (HEQ2: max (nl + m) (nr + p) + 2 =
                       S (S (max (nl + m) (nr + p))))
                  by omega; rewrite HEQ2; clear HEQ2;
        assert (HEQ3: (S (max (nl + m) (nr + p))) =
                       max (S (nl + m)) (S (nr + p)))
                  by eauto; rewrite HEQ3; clear HEQ3;
        assert (HEQ4: S (max (S (nl + m)) (S (nr + p))) =
                      (max (S (S (nl + m))) (S (S (nr + p)))))
                  by eauto; rewrite HEQ4; clear HEQ4;
                intuition.
   SCase "tr".
   inversion HTt1;subst.
   exists (nr + p + n + 1).
...
Lemma trunc_subtrac : forall n, 0 - n = 0.
Proof. firstorder with arith. Qed.
...
Theorem preservation' :
  forall t t' T n,
     has_type empty t T n ->
                     t ~> t' -> 
   exists m, (n - 1) <= m  /\ has_type empty t' T m.
Proof. 
  remember (@empty Ty) as Gamma.
  intros ? ? ? ? HTt HSt';
  generalize dependent t';
    has_type_cases (induction HTt) Case; intuition; intros; subst; try
    match goal with
    | [H: \empty ?y = Some ?T |- _] => inv H
    | [H: ?t ~> ?t' |- _] => inv H
    end;
    match goal with
    | [H: (forall (_ : term), ?x ~> _ -> _), H': (forall (_ : term), ?y ~> _ -> _)|- _] => rename H into IHHt1; rename H' into IHHt2
    | [H: (forall (_ : term), ?x ~> _ -> _) |- _ ] => rename H into IHHt1
    end;
    match goal with
    | [H: ?x ~> ?y, H': (forall (_ : term), ?x ~> _ -> _) |- _ ] => destruct (H' _ H) as [z [HLE HTy]]
    | _ => idtac
    end; try (exists z;split; [omega |]; try (eapply T_Inl); try (eapply T_Inr);eauto; fail);
     try (exists (z + max (nl + m) (nr + p) + 2);split; [omega |eapply T_Case;eauto];fail).
    inv HTt1.
        exists n. 
        split; [ | eapply values_subst_preserves_typing; eauto].
        assert (p = 0) by (eapply complexity_of_well_typed_values;eauto);subst. 
               intuition. 
        exists (n + z + p + 1); split;[omega|econstructor;eauto]. 
        assert (m = 0) by (eapply complexity_of_well_typed_values;eauto);subst. 
        exists (n + 0 + z + 1); split;[omega|econstructor;eauto].
        exists (z + m).
        split; [omega | eapply T_Pair; eauto].        
        exists (n + z).
        split; [omega | eapply T_Pair; eauto]. 
    SCase "T_Pi1E".
        inv HTt.
        assert (m = 0) by (eapply complexity_of_well_typed_values;eauto);subst. 
        assert (n0 = 0) by (eapply complexity_of_well_typed_values;eauto);subst. 
        exists 0.
        split; [omega|auto].
    SCase "T_Pi1". 
        exists (z + 1).
    ...
    SCase "tl".
        assert (n = 0) by (eapply complexity_of_well_typed_values;eauto); subst. 
  inversion HTt1;subst. 
  destruct (progress _ _ _ HTt2) as [HVal |HNVal].
           assert (m = 0) by (eapply complexity_of_well_typed_values;eauto); subst.
  exists (nl + 0 + 0 + 1).
  split; [ | eapply T_App;eauto].  
        simpl.
        assert (HEQ1: (nl + 0 + 0 + 1) = (nl + 1))
          by omega; rewrite HEQ1; clear HEQ1. simpl. 
        assert (HEQ2: (nl + 0 ) = nl)
          by omega; rewrite HEQ2; clear HEQ2. simpl.
        assert (HEQ3: max  nl (nr + p) + 2 - 1 = 
                      max  nl (nr + p) + 1) by omega; rewrite HEQ3; clear HEQ3. 
         Abort.
...
Proof.
  ...
  Case "T_Abs".
     ...
     pose proof (IHHT ((x,T1)::c) IHHT1 ((x,v)::env0) IHHT2) as HGoal2; simpl in HGoal2.   
     eapply nstep_preserves_R'; [econstructor;eauto|eapply HGoal1;eauto |eauto].
  Case "T_App".
    destruct (IHHT1 c H env0 V) as [_ [_ P1]]. 
    pose proof (IHHT2 c H env0 V) as P2. auto. 
  Case "TNil".
    split;[eexists;eapply T_Nil;eauto |split;[eapply values_terminate;eauto|auto]].
  Case "TProd".
    pose proof (IHHT1 c H env0 V) as HR1; 
    pose proof (IHHT2 c H env0 V) as HR2.
  ...
  Case "T_Pi1".
    pose proof (IHHT c H env0 V) as HR1.
    destruct (R_typable_empty _ _ HR1) as [? ?]; 
    destruct (R_terminates _ _ HR1).
    edestruct HR1 as [? [? [? [? [? [? [? [? [? ?]]]]]]]]].
  ...
  Case "T_Case".
    edestruct (IHHT2 c H env0 V) as [[l HTl] [HTerm1 Hlapp]];
         destruct HTerm1.
    edestruct (IHHT3 c H env0 V) as [[r HTr] [HTerm2 Hrapp]];
         destruct HTerm2.
    edestruct (IHHT1 c H env0 V) as [[s HTs] [HTerm3 [t' [o [HVt' [[HSUM HRs] | [HSUM HRs]]]]]]];
         destruct HTerm3;
         destruct (R_typable_empty _ _ HRs) as [s' HTs'].
    SCase "left".
      eapply nstep_preserves_R'.
        eapply T_Case; eauto.
